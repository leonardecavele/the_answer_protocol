...
impl GameComponent {
    ...
    fn get_available_actions(&self, _entity_name: &str) -> Vec<&'static str> {
        // TODO: Dynamically fetch actions based on entity type
        vec!["Look", "Talk", "Attack"]
    }
}
...
impl Component for GameComponent {
        ...
        if let Event::Key(key) = event {
            if self.context_menu_open {
                match key.code {
                    ...
                    KeyCode::Down => {
                        let entity_name = state.game.npcs_in_room.get(self.selected_entity_idx.unwrap_or(0)).cloned().unwrap_or_default();
                        let max_idx = self.get_available_actions(&entity_name).len().saturating_sub(1);
                    ...
                    KeyCode::Enter => {
                        ...
                        if let Some(action) = actions.get(self.context_menu_idx) {
                            let cmd = format!("{} {}", action.to_lowercase(), entity_name.to_lowercase());
            ...
            match key.code {
                ...
                KeyCode::Up => match self.focus {
                    ...
                    GameFocus::SystemLogs => {
                        state.ui.logger_state.transition(tui_logger::TuiWidgetEvent::UpKey);
                    }
                },
                KeyCode::Down => match self.focus {
                    ...
                    GameFocus::SystemLogs => {
                        state.ui.logger_state.transition(tui_logger::TuiWidgetEvent::DownKey);
                    }
                },
                KeyCode::Left => match self.focus {
                    ...
                    GameFocus::SystemLogs => state.ui.logger_state.transition(tui_logger::TuiWidgetEvent::LeftKey),
                    GameFocus::Input => { tui_input::backend::crossterm::EventHandler::handle_event(&mut self.input, event); }
                },
                KeyCode::Right => match self.focus {
                    GameFocus::Scene => {
                        handle_command(state, "move east".to_string(), tx.clone());
                    }
                    GameFocus::SystemLogs => state.ui.logger_state.transition(tui_logger::TuiWidgetEvent::RightKey),
                    GameFocus::Input => { tui_input::backend::crossterm::EventHandler::handle_event(&mut self.input, event); }
                },
                KeyCode::PageUp => {
                ...
                _ => {
                    if matches!(self.focus, GameFocus::Input) {
                        tui_input::backend::crossterm::EventHandler::handle_event(&mut self.input, event);
                    }
                }
            }
    ...
    fn draw(&mut self, state: &mut AppState, f: &mut Frame, area: Rect) {
        ...
        let header_str = format!(
            ...
            state.game.hp, state.game.max_hp,
            state.game.online_players
        );
        let header_widget = Paragraph::new(header_str).style(Style::default().bg(Color::DarkGray).fg(Color::White)).alignment(Alignment::Center);
        ...
        // Scene
        let (scene_border, scene_style) = if matches!(self.focus, GameFocus::Scene) { 
            (focused_color, Style::default().fg(focused_color).add_modifier(Modifier::BOLD)) 
        } else { 
        ...
        let scene_block = Block::default().borders(Borders::ALL).border_style(scene_style).title(scene_title);
        ...
        if let Some(dyn_img) = state.assets.get_image_for_context(check_room, &state.game.npcs_in_room) {
            let img_widget = ratatui_image::StatefulImage::new();
            f.render_stateful_widget(img_widget, inner_scene, dyn_img);
        } else {
            f.render_widget(Paragraph::new(">> IMAGE NOT FOUND / FAILED TO LOAD <<").style(Style::default().fg(Color::Red)), inner_scene);
        }
        ...
        if matches!(self.focus, GameFocus::Scene) {
            let north = Paragraph::new("[N]orth").style(Style::default().fg(Color::Yellow).bg(Color::Black));
            let south = Paragraph::new("[S]outh").style(Style::default().fg(Color::Yellow).bg(Color::Black));
            let east = Paragraph::new("[E]ast").style(Style::default().fg(Color::Yellow).bg(Color::Black));
            let west = Paragraph::new("[W]est").style(Style::default().fg(Color::Yellow).bg(Color::Black));
            
            // Positioning arrows manually over the inner scene
            let mut n_area = inner_scene;
            n_area.height = 1; n_area.width = 7; n_area.x = inner_scene.x + (inner_scene.width / 2).saturating_sub(3); n_area.y = inner_scene.y;
            
            let mut s_area = inner_scene;
            s_area.height = 1; s_area.width = 7; s_area.x = inner_scene.x + (inner_scene.width / 2).saturating_sub(3); s_area.y = inner_scene.y + inner_scene.height.saturating_sub(1);
            
            let mut e_area = inner_scene;
            e_area.height = 1; e_area.width = 6; e_area.x = inner_scene.x + inner_scene.width.saturating_sub(6); e_area.y = inner_scene.y + (inner_scene.height / 2);
            
            let mut w_area = inner_scene;
            w_area.height = 1; w_area.width = 6; w_area.x = inner_scene.x; w_area.y = inner_scene.y + (inner_scene.height / 2);
        ...
        if !state.game.npcs_in_room.is_empty() {
            let entities_str = state.game.npcs_in_room.iter().enumerate().map(|(i, n)| {
            ...
            let entities_p = Paragraph::new(format!(" Entities: {} ", entities_str))
                .style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD));
            f.render_widget(entities_p, Rect {
                x: inner_scene.x,
        ...
        if self.context_menu_open {
            if let Some(idx) = self.selected_entity_idx {
                if let Some(entity_name) = state.game.npcs_in_room.get(idx) {
                    let actions = self.get_available_actions(entity_name);
                    let items: Vec<ListItem> = actions.iter().enumerate().map(|(i, a)| {
                        if i == self.context_menu_idx {
                            ListItem::new(format!("> {}", a)).style(Style::default().fg(Color::Yellow))
                        } else {
                            ListItem::new(format!("  {}", a))
                        }
                    }).collect();
                    let menu = List::new(items).block(Block::default().borders(Borders::ALL).title(" Action "));
        ...
        // Game Events
        let game_lines: Vec<Line> = state.game.game_output.iter().map(|l| Line::from(l.as_str())).collect();
        let max_scroll = (game_lines.len() as u16).saturating_sub(left_chunks[1].height.saturating_sub(2));
        ...
        let messages_widget = Paragraph::new(game_lines)
            .block(Block::default().borders(Borders::ALL).border_style(Style::default().fg(unfocused_color)).title(" Game Events "))
            .scroll((scroll, 0))
            .wrap(Wrap { trim: true });
        ...
        let input_style = if matches!(self.focus, GameFocus::Input) { 
            Style::default().fg(focused_color).add_modifier(Modifier::BOLD)
        } else { 
            Style::default().fg(unfocused_color)
        };
        let input_widget = Paragraph::new(self.input.value())
            .block(Block::default().borders(Borders::ALL).border_style(input_style).title(" Command Input [Tab to switch] "))
        ...
        if matches!(self.focus, GameFocus::SystemLogs) {
            ...
            let logs_widget = tui_logger::TuiLoggerWidget::default()
                .block(Block::default().borders(Borders::ALL).border_style(Style::default().fg(focused_color).add_modifier(Modifier::BOLD)).title(" System Logs [Tab to hide] "))
